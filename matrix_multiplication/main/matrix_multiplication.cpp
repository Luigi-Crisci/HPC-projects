...
void matrix_sum_in_position(Int_Matrix A, Int_Matrix B, Int_Matrix* result, int x_start,int y_start)
{
	...
	for (long unsigned int i = 0; i < A.size(); i++)
		for (long unsigned int j = 0; j < A[0].size(); j++)
			(*result)[i + x_start][j + y_start] = A[i][j] + B[i][j];
}
										   ...
										   int B_row_start, int B_column_start, int size)
{
	...
	#pragma omp task shared(sum_result,l_l_h, r_l_h) depend(in: l_l_h,r_l_h) 
	matrix_sum_in_position(l_l_h, r_l_h,&sum_result,0,0);
	#pragma omp task shared(sum_result,l_l_l, r_l_l) depend(in: l_l_l,r_l_l)
	matrix_sum_in_position(l_l_l, r_l_l,&sum_result,diff,0);
	#pragma omp task shared(sum_result,l_r_h, r_r_h) depend(in: l_r_h,r_r_h)
	matrix_sum_in_position(l_r_h, r_r_h, &sum_result,0,diff);
	#pragma omp task shared(sum_result,l_r_l, r_r_l) depend(in: l_r_l,r_r_l)
	matrix_sum_in_position(l_r_l, r_r_l,&sum_result,diff,diff);
...
Int_Matrix recursive_tiling_multiplication_full(Int_Matrix A, Int_Matrix B)
{
	...
	#pragma omp parallel
	{
		#pragma omp single
		{
			C = recursive_tiling_multiplication(A, B, 0, 0, 0, 0, A.size());
		}
	}
	return C;
}
